<template>
  <div class="a-empty" :class="emptyClass">
    <div class="a-empty-image">
      <slot name="image">
        <svg v-if="size === 'small'" width="64" height="41" viewBox="0 0 64 41">
        ...
        <svg v-else width="184" height="152" viewBox="0 0 184 152">
    ...
    <div v-if="description || $slots.description" class="a-empty-description">
      <slot name="description">
        {{ description }}
      </slot>
    </div>
    
    <div v-if="$slots.default" class="a-empty-footer">
      <slot />
    </div>
  </div>
</template>
...
const emptyClass = computed(() => ({
  [`a-empty-${props.size}`]: props.size !== 'normal'
}))
</script>
...
.a-empty-small .a-empty-image {
  margin-bottom: 4px;
}

.a-empty-small .a-empty-description {
  margin-bottom: 8px;
  font-size: 12px;
}
</style>